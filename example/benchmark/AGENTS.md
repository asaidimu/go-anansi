...
Model and full modes embed `document.DocumentModel` and emit a `document.New` constructor (container-backed documents, matching the persistence layer's default). Full mode also emits a `DangerouslyReset<X>Model()` helper (marked `Deprecated`) that clears the cached singleton — after rebuilding the persistence layer in tests, call it so `Init<X>Model` can construct a fresh instance.
