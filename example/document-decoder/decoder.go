...
func DecodeJSON(cs *definition.CompiledSchema, data []byte) (*container.DataContainer, error) {
	...
	p.skipWS()
	if !p.eof() {
		return nil, fmt.Errorf("document-decoder: trailing data after JSON document")
	}
	return doc, nil
}
...
func DecodeJSONInto(cs *definition.CompiledSchema, data []byte, doc *container.DataContainer, pool *container.Pool) error {
	...
	p.skipWS()
	if !p.eof() {
		return fmt.Errorf("document-decoder: trailing data after JSON document")
	}
	return nil
}
...
func decodeObject(cs *definition.CompiledSchema, doc *container.DataContainer, schemaIdx uint8, path definition.ResolvedPath, p *jsonParser, pool *container.Pool, cache []container.DataContainerKey) error {
	if int(schemaIdx) >= len(cs.Schemas) {
		return fmt.Errorf("document-decoder: schema slot %d out of range", schemaIdx)
	}
	...
	for j := uint16(0); j < slot.FieldCount; j++ {
		...
		if fd.Required() {
			return fmt.Errorf("document-decoder: missing required field %q", meta.Name)
		}
		// THIS SHOULD BE CONFIGURABLE
		if fd.HasDefault() {
			step := definition.NewResolvedStep(schemaIdx, uint8(j))
			fieldPath := appendStep(path, step)
			if err := setLeaf(cs, doc, fd, abs, fieldPath, meta.Default.Value(), cache); err != nil {
				return err
			}
		}
	}
...
func parseLeaf(p *jsonParser, cs *definition.CompiledSchema, doc *container.DataContainer, fd definition.FieldDescriptor, abs int, path definition.ResolvedPath, cache []container.DataContainerKey) error {
	...
	return fmt.Errorf("document-decoder: unsupported data type %d", fd.DataType())
}

func setLeaf(cs *definition.CompiledSchema, doc *container.DataContainer, fd definition.FieldDescriptor, abs int, path definition.ResolvedPath, value any, cache []container.DataContainerKey) error {
	key, err := leafKey(cs, fd, abs, path, cache)
	if err != nil {
		return err
	}
	return setByType(doc, fd.DataType(), key, value)
}
...
func buildAddressCache(cs *definition.CompiledSchema, schemaIdx uint8, path definition.ResolvedPath, cache []container.DataContainerKey) error {
	if int(schemaIdx) >= len(cs.Schemas) {
		return fmt.Errorf("document-decoder: build address cache: schema slot %d out of range", schemaIdx)
	}
	slot := cs.Schemas[schemaIdx]
	for j := uint16(0); j < slot.FieldCount; j++ {
		abs := int(slot.FieldStart) + int(j)
		...
		if err != nil {
			return fmt.Errorf("document-decoder: build address cache for schema %d field %d: %w", schemaIdx, j, err)
		}
		cache[abs] = key
	}
	return nil
...
func computeLeafKey(cs *definition.CompiledSchema, fd definition.FieldDescriptor, path definition.ResolvedPath) (container.DataContainerKey, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("document-decoder: build data point: %w", err)
	}
...
func setByType(doc *container.DataContainer, dt container.DataType, key container.DataContainerKey, value any) error {
	...
	case container.TypeBool:
		b, ok := value.(bool)
		if !ok {
			return fmt.Errorf("document-decoder: expected boolean, got %T", value)
		}
		return doc.SetBool(key, b)
	case container.TypeBytes:
	...
	return fmt.Errorf("document-decoder: unsupported data type %d", dt)
}

func typeErr(path, want string, got any) error {
	return fmt.Errorf("document-decoder: field %q expects %s, got %T", path, want, got)
}
...
func asInt64(v any) (int64, error) {
	...
	return 0, fmt.Errorf("document-decoder: expected integer, got %T", v)
}
...
func asFloat64(v any) (float64, error) {
	...
	return 0, fmt.Errorf("document-decoder: expected number, got %T", v)
}
...
func asGeometry(v any) ([][]float64, error) {
	outer, ok := v.([]any)
	if !ok {
		return nil, fmt.Errorf("document-decoder: expected geometry (array of rings), got %T", v)
	}
	out := make([][]float64, len(outer))
	for i, ring := range outer {
		inner, ok := ring.([]any)
		if !ok {
			return nil, fmt.Errorf("document-decoder: geometry ring %d is not an array", i)
		}
		out[i] = make([]float64, len(inner))
		for j, c := range inner {
			f, err := asFloat64(c)
			if err != nil {
				return nil, fmt.Errorf("document-decoder: geometry ring %d coord %d: %w", i, j, err)
			}
			out[i][j] = f
		}
	}
...
func asInt64Slice(v any) ([]int64, error) {
	arr, ok := v.([]any)
	if !ok {
		return nil, fmt.Errorf("document-decoder: expected array, got %T", v)
	}
...
func asBoolSlice(v any) ([]bool, error) {
	...
	for i, e := range arr {
		b, ok := e.(bool)
		if !ok {
			return nil, fmt.Errorf("document-decoder: array element %d is not a boolean", i)
		}
		out[i] = b
	}
	return out, nil
...
func asGeometrySlice(v any) ([][][]float64, error) {
	arr, ok := v.([]any)
	if !ok {
		return nil, fmt.Errorf("document-decoder: expected array of geometry, got %T", v)
	}
	out := make([][][]float64, len(arr))
	for i, e := range arr {
		g, err := asGeometry(e)
