...
// CreateView creates a materialized view in the ephemeral store. The
// materialized view is stored as a regular collection whose rows are
// populated by executing the selectQuery against the underlying collections.
// For the ephemeral backend this is a no-op stub — materialized views are
// not fully supported in-memory. Returns ErrDDLNotSupported to signal
// callers that the ephemeral backend cannot materialize views.
func (m *EphemeralDatabaseInteractor) CreateView(ctx context.Context, name string, selectQuery *query.Query) error {
        return query.ErrDDLNotSupported
}

// RefreshView re-populates a materialized view's physical table. Not
// supported by the ephemeral backend.
func (m *EphemeralDatabaseInteractor) RefreshView(ctx context.Context, name string, selectQuery *query.Query) error {
        return query.ErrDDLNotSupported
}
