...
type SystemError struct {
        ...
        Retryable     bool   // True if the operation may succeed on retry (e.g., lock contention, connection hiccup).
        //                         The pipeline uses this to decide whether to automatically retry.
        //                         Dialect-specific error translators set this based on native error codes.
...
// WithRetryable returns a new SystemError with the retryable flag set.
// Use this in dialect-specific error translators to signal transient failures
// (lock contention, connection hiccup, MVCC conflict) vs permanent errors
// (constraint violation, schema mismatch, permission denied).
//
// The pipeline's retry mechanism checks this flag (via IsRetryable) to decide
// whether to automatically retry the operation. Permanent errors skip retry
// and propagate immediately.
func (e *SystemError) WithRetryable(retryable bool) *SystemError {
        newErr := *e
        newErr.Retryable = retryable
        return &newErr
}

// IsRetryable walks the error chain (this error and all wrapped causes) and
// returns true if any SystemError in the chain is marked Retryable.
// This lets the pipeline check any wrapped error without needing to understand
// dialect-specific error codes.
//
// The walk is iterative (not recursive) to avoid stack depth issues on
// deeply wrapped error chains.
func (e *SystemError) IsRetryable() bool {
        if e.Retryable {
                return true
        }
        cause := e.Cause
