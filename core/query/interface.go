...
// without hardcoding backend-specific behavior.
//
// These are hints, not guarantees — the pipeline uses them to set
// sensible defaults. Developers can override via SetupConfig.
type ConcurrencyModel struct {
        // ConcurrentReads indicates the backend can serve multiple readers
        // without external serialization. When false, the pipeline may
        // serialize reads through the write serialization mechanism.
        ConcurrentReads bool

        // ConcurrentWrites indicates the backend handles multiple concurrent
        // writers natively (e.g., MVCC). When false, the pipeline ensures
        // only one write transaction is in-flight at a time.
        ConcurrentWrites bool

        // WriteSerialization, when true, causes the pipeline to serialize all
        // write transactions with an exclusive lock. This is the mechanism
        // previously controlled by RequiresTransactionSerialization.
        //
        // For SQLite (single writer), this should be true.
        // For PostgreSQL (MVCC), this should be false.
        //
        // If RequiresTransactionSerialization is also set (backward compat),
        // WriteSerialization defaults to true regardless of this field.
        WriteSerialization bool

        // MaxConcurrentReadOps is a hint for connection pool sizing.
        // 0 means "unlimited" — the pipeline applies sensible defaults.
        // Backends that degrade under high read concurrency should set this
        // to a conservative value.
        MaxConcurrentReadOps int
}

type Capabilities struct {
        // RequiresTransactionSerialization forces all transactions to be serialized with a mutex.
        // Kept for backward compatibility. New code should use Concurrency.WriteSerialization.
        RequiresTransactionSerialization bool

        // Concurrency describes the backend's concurrency characteristics.
        // When set, it takes precedence over RequiresTransactionSerialization.
