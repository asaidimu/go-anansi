...
const (
        ...
        // StmtCreateView represents creating a materialized view table
        // (CREATE TABLE <name> AS SELECT ... in SQLite). The DSL query
        // carries the SELECT to materialize; the target name is the
        // materialized view's physical table name.
        StmtCreateView StatementType = "CREATE_VIEW"

        // StmtRefreshView represents refreshing a materialized view by
        // dropping and re-creating its physical table from the stored
        // SELECT. Equivalent to DROP TABLE + CREATE TABLE AS SELECT
        // in a single logical operation.
        StmtRefreshView StatementType = "REFRESH_VIEW"
)
