...
func (i *NativeInteractor[T]) createCollectionLogic(ctx context.Context, sc definition.Schema) error {
        ...
        for _, index := range sc.Indexes {
                ...
                // Full-text indexes are emitted inline by the CREATE TABLE DDL
                // (the SQLite factory's createTableTree auto-emits FTS5 virtual
                // tables and sync triggers for every IndexTypeFullText entry).
                // Calling CreateIndex here would attempt to create the FTS table
                // a second time; skip it to avoid duplicate-creation errors.
