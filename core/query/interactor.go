...
type SchemaManager interface {
        ...
        // CreateView creates a materialized view: a physical table populated
        // by executing the given SELECT query. The table is named after the
        // view's physical name (derived by the registry) and its column shape
        // is determined by the SELECT's projection.
        //
        // For SQL backends this is typically `CREATE TABLE <name> AS SELECT ...`.
        // The view's indexes (declared on its derived schema) are created
        // separately via CreateIndex after the table is populated.
        //
        // The query's Target.Name carries the underlying collection's
        // PHYSICAL name (already resolved by the caller); joins in the query
        // reference their physical names too. The caller is responsible for
        // resolving logical → physical names before calling CreateView.
        CreateView(ctx context.Context, name string, selectQuery *Query) error

        // RefreshView drops the materialized view's physical table and
        // re-creates it from the stored SELECT. Reads against the view
        // between drop and re-create will fail; callers that need
        // zero-downtime refresh should wrap this in a swap-two-tables
        // pattern at the application layer.
        RefreshView(ctx context.Context, name string, selectQuery *Query) error
}
