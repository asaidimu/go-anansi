...
// EventBus defines the interface for an event bus that can emit and subscribe to persistence events.
type EventBus[T any] interface {
	Emit(eventType string, event T)
	// Subscribe registers a handler for a persistence event, with an optional filter function.
