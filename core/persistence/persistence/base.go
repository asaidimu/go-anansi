...
func (p *basePersistence) TransactOpts(ctx context.Context, opts base.TransactOpts, callback func(ctx context.Context, tx base.BasePersistence) (any, error)) (any, error) {
        ...
        // Nested transactions or backends that don't need serialization: no mutex.
        if _, ok := transaction.GetCurrentTransaction(ctx); ok || !needsSerialization {
                return execute()
        }

        // Serialization required: acquire the mutex, execute, release.
        // The retry (if enabled) happens INSIDE execute(), but because retry
        // backoff sleeps, we use a semaphore-style approach: acquire only for
        // the decision, not for the full retry duration.
        //
        // With IMMEDIATE transactions + busy_timeout, SQLite handles contention
        // at the native level, making this mutex largely redundant. It's kept
        // as a safety net for backends that don't have native contention handling.
        p.txMu.Lock()
        defer p.txMu.Unlock()
