...
func (p *registrySchemaProvider) IsView() bool {
        // Resolve the entry to check whether the active version is a view.
        // We use a background context here because IsView is called from
        // hot paths (Create/Update/Delete/Validate) where we don't want to
        // inherit a request-scoped deadline that might be cancelled. The
        // registry is cache-backed so this is fast in the steady state.
