...
// GenerateMetadataFiles renders metadata.go (always) and providers.go
// (write-once; stubs for newly declared providers are appended) into the
// configured output directory. It returns the list of files written.
func GenerateMetadataFiles(cfg *Config, m *Metadata, dryRun bool) ([]string, error) {
	...
	provWritten, err := ensureProviderStubs(providersPath, pkg, m, dryRun)
	if err != nil {
		return nil, err
	}
	written = append(written, provWritten...)
	return written, nil
}
...
`, jsonLiteral(mergedJSON), jsonLiteral(depsJSON))
	...
	b.WriteString("// MetadataProviderConfigs returns the entries for DocumentFactoryConfig.Providers,\n")
	b.WriteString("// one per provider declared in metadata.schema.json. Each entry carries the\n")
	b.WriteString("// provider's generated schema and dependencies plus the provider func from\n")
	b.WriteString("// providers.go — assign it straight to the factory config:\n")
	b.WriteString("//\n")
	b.WriteString("//\tcfg.Providers = migrations.MetadataProviderConfigs()\n")
	b.WriteString("func MetadataProviderConfigs() []data.MetadataProviderConfig {\n")
	b.WriteString("\treturn []data.MetadataProviderConfig{\n")
	for _, p := range m.Providers {
		fmt.Fprintf(&b, "\t\t{Name: %q, Schema: %sSchema(), Dependencies: MetadataDependencies(), Provider: %sProvider},\n",
...
func renderProvidersGo(pkg string, m *Metadata) string {
	...
	b.WriteString("// This file contains provider stubs. Stub bodies are preserved across\n")
	b.WriteString("// regenerations; edit the bodies to produce the metadata declared for each\n")
	b.WriteString("// provider. New providers get a stub appended on the next run.\n\n")
...
`)
	for _, p := range m.Providers {
		b.WriteString(providerStub(p))
	}
	return b.String()
}

// ensureProviderStubs guarantees providers.go has a stub for every provider in
// m. On first creation the whole file is rendered. When the file already
// exists its content is preserved verbatim and stubs for providers whose
// function is missing are appended — existing (possibly user-edited) bodies are
// never touched. Returns the files written in this call.
func ensureProviderStubs(path, pkg string, m *Metadata, dryRun bool) ([]string, error) {
	...
	if os.IsNotExist(err) {
		...
		if err := os.WriteFile(path, []byte(renderProvidersGo(pkg, m)), 0644); err != nil {
	...
	src := string(data)
	var missing []MetadataProvider
	for _, p := range m.Providers {
		if strings.Contains(src, "func "+p.Ident+"Provider(") {
			continue
		}
	...
	if dryRun {
		fmt.Printf("  would add provider stubs: %s\n", path)
		return nil, nil
	}
	if err := os.WriteFile(path, []byte(src+b.String()), 0644); err != nil {
		return nil, fmt.Errorf("append stubs to %s: %w", path, err)
	}
	fmt.Printf("  added provider stubs: %s\n", path)
	return []string{path}, nil
}

// providerStub renders the implementation stub for a single provider.
func providerStub(p MetadataProvider) string {
	var b strings.Builder
