...
func (t *createTableTree) Value() (string, []any, error) {
        ...
        // Auto-emit FTS5 virtual tables for any IndexTypeFullText indexes
        // declared on the schema. Each FTS table is created with sync triggers
        // so writes to the main collection table keep the FTS index up to date.
        for _, idx := range collectFullTextIndexes(t.schema) {
                ftsTree := &createFTSTree{
                        collection: t.schema.Name,
                        schema:     t.schema,
                        index:      idx,
                }
                ftsSQL, _, err := ftsTree.Value()
                if err != nil {
                        return "", nil, err
                }
                sb.WriteString("\n")
                sb.WriteString(ftsSQL)
        }
...
func (f *sqliteFactory) buildDropTableTree(q *query.Query) (SQLNode, error) {
        return &dropTableTree{name: q.Target.Name, schema: q.Target.Schema}, nil
}
...
func (t *dropTableTree) Value() (string, []any, error) {
        ...
        // Drop any FTS5 virtual tables + sync triggers declared on the
        // collection before dropping the underlying table. The triggers must
        // be removed first; otherwise dropping the FTS table would fire them
        // against a half-dropped collection.
        if t.schema != nil {
                for _, idx := range collectFullTextIndexes(t.schema) {
                        drop := &dropFTSTree{collection: t.name, schema: t.schema, index: idx}
                        sql, _, err := drop.Value()
                        if err != nil {
                                return "", nil, err
                        }
                        sb.WriteString(sql)
        ...
        sb.WriteString(fmt.Sprintf("DROP TABLE IF EXISTS %s;", quoteIdentifier(t.name)))
        return sb.String(), nil, nil
}
