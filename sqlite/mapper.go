...
type SqliteInteractorOptions struct {
		IfNotExists  bool // Prevent errors if a table already exists.
		CreateIndexes bool // Automatically create indexes defined in the schema.
}
...
func DefaultInteractorOptions() *SqliteInteractorOptions {
	return &SqliteInteractorOptions{
...
func (s *SQLiteInteractor) getTableName(baseName string) string {
	return s.quoteIdentifier(baseName)
}
...
func (s *SQLiteInteractor) CollectionExists(collection string) (bool, error) {
	...
	var name string
	err := s.runner().QueryRow(query, collection).Scan(&name)
	if err != nil {
