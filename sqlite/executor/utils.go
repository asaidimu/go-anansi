...
//
// The map keys are the raw SQL column names from rows.Columns(). For single-table
// queries these are bare field names (e.g. "id"). For join queries SQLite
// automatically prefixes columns only when there is a naming ambiguity between
// tables (e.g. "users.id" vs "orders.id"); non-ambiguous columns remain bare.
func ReadRows(ctx context.Context, logger *zap.Logger, sc *definition.Schema, rows *sql.Rows) ([]*document.Document, int64, error) {
	...
	if sc == nil {
		...
		processRow = func(row map[string]any) map[string]any {
			...
			for col, value := range row {
				...
				// Use the raw SQL column name as the map key. SQLite already
				// handles disambiguation — bare for unambiguous columns,
				// table-qualified when two tables share a column name.
				_, fieldDef := sc.FindField(col)
				cv, err := fromSQLiteValue(fieldDef, value)
				if err != nil {
					logger.Warn("failed to convert value", zap.String("field", col), zap.Error(err))
					flat[col] = value
				} else {
					flat[col] = cv
				}
	...
	for row := range utilDocChan {
		...
		results = append(results, document.Record(processRow(row), ctx))
	}
