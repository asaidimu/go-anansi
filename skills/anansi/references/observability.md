# Observability, realtime, and lifecycle hygiene

Read this when you're **operating or hardening** a running app: exporting
metrics, wiring events for observability, building realtime features, and the
goroutine/cache cleanup checklist. Verified against the framework source.
...
- `LiveCollection` is the server-side "reactive view" half: a processor that
  ...
  `references/caching.md` for the full `LiveCollection` contract.)
...
- `Release()` documents you hold from the persistence layer; let the
  ...
  `references/caching.md` for what failing to `Release` costs.)