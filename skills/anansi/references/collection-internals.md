# What happens between your request and the database

This file answers the debugging question that decides whether a bug lives in
*your code* or in the framework: **when you call a method on a collection,
what exactly runs, in what order, before the database is touched — and where
do the errors you see actually come from?**

Every layer below is real code in the framework, verified against source. Read
...
call site or file a framework issue.
...
bug is in your code or the framework.
...
| `ERR_PERSISTENCE_INSERT/READ/UPDATE/DELETE_DOCUMENTS_FAILED` | baseCollection | The interactor call failed after your input passed all checks. **Framework or driver** — look at the wrapped cause. |
...
| `ErrRecordNotFound` | ModelCollection | Cache negative hit, or read-back came back empty. **Your id / expectations** — unless cache is stale (see cache semantics in `references/caching.md`). |
...
framework layers; errors like `ERR_VALIDATION_*`, `ERR_QUERY_*`, and the
safety guards are usually your input's fault.** If the wrapped cause chain ends
at the interactor/driver, check the SQL/data; if it ends in a framework
...
`references/persistence-setup.md`); `DocumentPool(ctx)` → the schema-bound container pool (lazily compiled,
