...
pairs with `references/schema-format.md` (the schema JSON format) and
`references/collection-internals.md`/`references/caching.md` (the
...
- **`composite`** = "this field is **two or more** named schema fragments
  ...
  framework's own meta-schema does exactly this: `Constraint` is declared
...
framework's own example apps and integration-test schemas, `union` and
...
`Validate` hook (`references/decorators.md`) is often the pragmatic default
until you do.
...
- **Mechanical domain / raw docs.** The hot path is numbers over many rows.
  ...
  to `float64` is exactly the precision loss the framework's `decimal` type
  exists to prevent).
...
- `references/collection-internals.md` — the request path layer-by-layer,
  `DocumentPool`/`Read`/`Release`, and a debugging table.
- `references/caching.md` — the `ModelCollection` id-cache fast path and the
  document `Release()`/pooling contract.
- `references/query-dsl.md` — the fluent `QueryBuilder`, shaping/filtering
  collections.
