...
description: "Custom document metadata — declare, regenerate, implement the stub, wire into data.ConfigureDocumentFactory."
---
...
document finalization. Verified against the source
...
for production; see `/reference/schema-rules` §3.)
...
> the library's own `DocumentFactoryConfig` is a normal struct you build and
