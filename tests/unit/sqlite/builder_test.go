...
func TestSelectWithInAndNinOperators(t *testing.T) {
	...
	expectedSQLIn := `SELECT "region" AS 'region', "status" AS 'status' FROM "users" WHERE "region" IN ($1, $2)`
	...
	// Fields in alphabetical order: region, status
	expectedSQLNotIn := `SELECT "region" AS 'region', "status" AS 'status' FROM "users" WHERE "status" NOT IN ($1, $2)`
...
func TestSelectWithDifferentDataTypesInWhere(t *testing.T) {
	...
	expectedSQLInt := `SELECT "age" AS 'age' FROM "users" WHERE "age" > $1`
	...
	expectedSQLFloat := `SELECT "in_stock" AS 'in_stock', "price" AS 'price' FROM "products" WHERE "price" <= $1`
	...
	expectedSQLBool := `SELECT "in_stock" AS 'in_stock', "price" AS 'price' FROM "products" WHERE "in_stock" = $1`
...
func TestSelectWithDistinct(t *testing.T) {
	...
	expectedSQLAll := `SELECT DISTINCT "city" AS 'city', "country" AS 'country' FROM "users"`
...
func TestSQLiteFactory_SelectImplicitFields(t *testing.T) {
	...
	expectedSQL := `SELECT "balance" AS 'balance', "id" AS 'id', "name" AS 'name' FROM "accounts" WHERE "id" = $1`
	assert.Equal(t, expectedSQL, nq.Raw().SQL)
	assert.Equal(t, 1, len(nq.Raw().Params))
