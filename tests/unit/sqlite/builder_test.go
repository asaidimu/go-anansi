...
func TestSelectWithInAndNinOperators(t *testing.T) {
	...
	assert.ElementsMatch(t, []interface{}{"East", "West"}, nqIn.Raw().Params)
	...
	assert.ElementsMatch(t, []interface{}{"inactive", "pending"}, nqNotIn.Raw().Params)
}
...
func TestSelectWithCaseStatement(t *testing.T) {
	...
	assert.Equal(t, expectedSQL, nq.Raw().SQL)
	assert.Equal(t, []interface{}{"active", "Active User", "inactive", "Inactive User", "Unknown Status"}, nq.Raw().Params)
}
...
func TestSelectWithDifferentDataTypesInWhere(t *testing.T) {
	...
	assert.Equal(t, []interface{}{float64(30)}, nqInt.Raw().Params)
	...
	assert.Equal(t, []interface{}{9.99}, nqFloat.Raw().Params)
	...
	assert.Equal(t, []interface{}{true}, nqBool.Raw().Params)
}
