...
func setupCollection(t *testing.T) (base.Collection, query.DatabaseInteractor, *zap.Logger, *definition.Schema, *goevents.EventBus, context.Context) {
        ...
        c, err := collection.NewCollection(eventEmitter, testSchemaDef.Name, collection.NewStaticSchemaProvider(testSchemaDef), ephemeralInteractor, engine, logger, resolveSchema, nil, nil)
        assert.NoError(t, err)
...
func setupNonExistentCollection() (base.Collection, query.DatabaseInteractor, *zap.Logger, *definition.Schema, *goevents.EventBus, context.Context) {
        ...
        eventEmitter := cevents.NewEventEmitter(pevents.NewGoEventsBusAdapter[persistence.PersistenceEvent](bus), factory.CreateEvent, logger)
        c, _ := collection.NewCollection(eventEmitter, nonExistentSchema.Name, collection.NewStaticSchemaProvider(nonExistentSchema), ephemeralInteractor, engine, logger, resolveSchema, nil, nil)
        ctx := context.Background()
...
func TestCollection_Read(t *testing.T) {
        ...
        t.Run("read documents error - non-existent collection", func(t *testing.T) {
                ...
                factory := pevents.NewPersistenceEventFactory(nonExistentSchema.Name, logger)
                eventEmitter := cevents.NewEventEmitter(pevents.NewGoEventsBusAdapter[persistence.PersistenceEvent](bus), factory.CreateEvent, logger)
                nonExistentCollection, _ := collection.NewCollection(eventEmitter, nonExistentSchema.Name, collection.NewStaticSchemaProvider(nonExistentSchema), ephemeralInteractor, engine, logger, resolveSchema, nil, nil)
