...
*Generated Output*: `User` contains `created_at`, `updated_at`, and `username` directly in its `fields` dictionary, in that order. Note that `DocumentModel`'s `_id_` and `_metadata_` fields are excluded from schema extraction (tagged `anansi:"-"`); only user‑defined embedded structs are promoted. No separate schema is created for `AuditMixin` (unless it is referenced elsewhere as its own named type, in which case it's discovered and registered independently on that reference, unaffected by the promotion happening here).
